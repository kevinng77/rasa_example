...
class DIETClassifier(GraphComponent, IntentClassifier, EntityExtractorMixin):
    ...
    def _predict(
        ...
        # import pdb
        # pdb.set_trace() 
        # logger.info(f"model_data feature length, {model_data.data['text']['sentence'].shape}")
        # model_data
        if model_data.is_empty():
            return None
    ...
    def process(self, messages: List[Message]) -> List[Message]:
        ...
        for message in messages:
            ...
            # TODO
            #     logger.info(f"out 'text_transformed' \
            # {out.get('diagnostic_data')['text_transformed'].shape}")
            #  (1, 3, 256)  [bathc size, 1 + len_seq, hidden_size]
            # logger.info(f"out keys \
            # {out.get('diagnostic_data').keys()}")
            # out: {'i-score:[], 'diagnostic_data'}
            # dict_keys(['attention_weights', 'text_transformed'])
            ...
            if self.component_config[ENTITY_RECOGNITION]:
                ...
                logger.info(f"Entity set: {entities}")
